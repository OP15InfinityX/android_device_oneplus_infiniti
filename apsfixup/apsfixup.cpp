...
//
// Loaded into com.oplus.camera as a DT_NEEDED of /odm/lib64/libAlgoProcess.so. Offsets are pinned
// to the infiniti (OnePlus 15, SM8850) odm blobs via readelf R_AARCH64_JUMP_SLOT offsets (host
// static analysis; no runtime/frida discovery needed):
//   libAlgoProcess.so    p010LSB2MSBNeon GOT slot @ +0x689ba8   (FUNC @ +0x4fc094 in .201 / +0x4fc25c in .300)
//   libAlgoInterface.so  dlsym@LIBC GOT slot      @ +0x1bb67c8
//
// NO BuildId guard (dodge-faithful). The GOT-slot offsets above are STABLE across the OOS .201/.300
// libAlgoProcess builds; only the p010 FUNC vaddr moved, and that is used solely for a cosmetic
// "blob drift?" LOGW, never to gate the hook. A prior version added a build_id_matches() guard that
// refused to redirect unless the loaded blob's NT_GNU_BUILD_ID matched a hardcoded id; whenever the
// shipped odm blob and the anchored id disagree (as happened with a stale .201 blob vs a .300 anchor)
// it made the shim PERMANENTLY INERT on the flashed image (no p010/chroma fix -> capture crash).
// Dropping the guard restores dodge behaviour: redirect the (version-stable) GOT slot and store
// whatever real function pointer it holds. The proprietary blobs are now synced to OOS .300
// (libAlgoProcess BuildId 2217d555); the GOT slots are unchanged, so the redirect is correct either way.
//
...
static const uintptr_t P010_FUNC_OFF = 0x4fc25c;   // p010LSB2MSBNeon in the .300 blob (2217d555); cosmetic drift-check only
static const uintptr_t P010_GOT_OFF  = 0x689ba8;   // its JUMP_SLOT GOT entry in libAlgoProcess (stable .201/.300)
static const uintptr_t DLSYM_GOT_OFF = 0x1bb67c8;  // dlsym@LIBC JUMP_SLOT GOT entry in libAlgoInterface (stable .201/.300)
