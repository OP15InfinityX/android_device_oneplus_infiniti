...
// hook the REAL p010LSB2MSBNeon (libAlgoProcess+0x4fa23c) and CLAMP its length so it converts
...
function armConv(){
  ...
  try{
    Interceptor.attach(m.base.add(0x4fa23c), {
      onEnter:function(a){
        var src=a[1];
    ...
    convArmed=true;
    console.log('[*] hooked p010LSB2MSBNeon @ '+hx(m.base.add(0x4fa23c))+' (clamp to buffer)');
